...
class MySQLFetcher(FragmentDataFetcher):
    ...
    # TODO: Optimize for larger data sets
    name = 'mysql'
    mutex = Lock()
...
def _build_sql_query(doc_var, fetcher_info, metadata):
    if fetcher_info.get('raw_query'):
        sql_string = fetcher_info['raw_query']
    else:
        select_clause = _create_select_clause(
                                        fetcher_info,
                                        metadata)
        from_clause = _create_from_clause(
                                        fetcher_info,
                                        metadata)
        ...
        if select_join_clause:
            select_clause = ', '.join([select_clause,
                                       select_join_clause])
        where_clause = _create_where_clause(
                                        doc_var,
                                        fetcher_info,
                                        metadata)
...
def _create_where_clause(doc_var, context, metadata):
    ...
    filter_clause = []
    filter_clause.extend(_build_filter_term(
                            context,
                            column_aliases,
                            doc_var,
                            metadata,
...
__all__ = ['MySQLFetcher', 'SQLiteFetcher', ]
