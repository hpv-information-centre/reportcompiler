        ...
        try:
            ...
            try:
                r_code =    """library(knitr);\
                            setwd('{}');\
                            knitr::knit(input='{}', encoding='UTF-8');\
                            """.format(context['meta']['tmp_path'].replace('\\', '\\\\'),
                                                             os.path.join(context['meta']['tmp_path'], 'templates', main_template).replace('\\', '\\\\'),
                                                           )
                ...
                TemplateRenderer.raise_rendering_exception(e, context, message=e.stdout)
        ...
        for template_file, dict_path in context_info:
            with open(os.path.join(context['meta']['templates_path'], template_file), 'r') as f_orig, \
                ...
                if template_file == context['meta']['main_template']:
                    metadata, content = re.findall(pattern='[ ]*(---.*?---)(.*)', string=content, flags=re.DOTALL)[0]
                    header = """
                             ...
                             context = fromJSON('{}')
                             ```
                             """.format(json.dumps(doc_var), json.dumps(context).replace('\\','\\\\'))
                    content = '\n'.join([metadata, header, content])
                f_tmp.write(content)


    def included_templates(self, content):
        matches = re.findall(pattern='```{.*child[ ]*=[ ]*[\'"](.*?)[\'"]', string=content)
